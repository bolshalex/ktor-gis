...
set CLASSPATH=%APP_HOME%\lib\ktor-map-backend-all.jar

@rem Execute ktor-map-backend
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %KTOR_MAP_BACKEND_OPTS%  -jar "%CLASSPATH%" %CMD_LINE_ARGS%
...
if  not "" == "%KTOR_MAP_BACKEND_EXIT_CONSOLE%" exit 1
exit /b 1
